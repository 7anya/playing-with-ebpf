...
program = r"""
BPF_PROG_ARRAY(syscall, 300); // defines map caled syscall with capcity of 300 entries

int hello(struct bpf_raw_tracepoint_args *ctx) { //2  ctx passed to an eBPF program attached to a raw tracepoint
    int opcode = ctx->args[1];// 3 // opcode to which syscall was made
    syscall.call(ctx, opcode);//4 // call the eBPF program associated with the opcode
    bpf_trace_printk("Another syscall: %d", opcode);//5 if tail call succeeds we wont see this line print execute
    return 0;
}
